...
plt.tight_layout()
plt.savefig(os.path.join(output_dir, 'mc_ac_diagram.svg'), format='svg', transparent=True)
plt.close()
...
plt.tight_layout()
plt.savefig(os.path.join(output_dir, 'mpl_apl_diagram.svg'), format='svg', transparent=True)
plt.close()
...
ax.text(2.2, 3, r'$|E_p| > 1 \Rightarrow MR > 0$' + '\n(Cầu co giãn nhiều)', fontsize=11, fontweight='bold', color='#1e40af')
ax.text(6.2, 2, r'$|E_p| < 1 \Rightarrow MR < 0$' + '\n(Cầu ít co giãn)', fontsize=11, fontweight='bold', color='#991b1b')
...
plt.tight_layout()
plt.savefig(os.path.join(output_dir, 'elasticity_mr_diagram.svg'), format='svg', transparent=True)
plt.close()
...
ax.text(0.2, 0.5, 'THU NHẬP\nQUỐC DÂN (Y)', fontsize=12, fontweight='bold', ha='center', va='center',
        bbox=dict(boxstyle='round,pad=0.6', facecolor='#f0fdf4', edgecolor='#16a34a', lw=1.5))

ax.text(0.8, 0.68, 'TIÊU DÙNG (C)\n' + r'$MPC = \frac{\mathrm{d}C}{\mathrm{d}Y}$', fontsize=12, fontweight='bold', ha='center', va='center',
        bbox=dict(boxstyle='round,pad=0.6', facecolor='#f0f9ff', edgecolor='#0284c7', lw=1.5))

ax.text(0.8, 0.32, 'TIẾT KIỆM (S)\n' + r'$MPS = \frac{\mathrm{d}S}{\mathrm{d}Y} = 1 - MPC$', fontsize=12, fontweight='bold', ha='center', va='center',
        bbox=dict(boxstyle='round,pad=0.6', facecolor='#fffbeb', edgecolor='#d97706', lw=1.5))
...
plt.tight_layout()
plt.savefig(os.path.join(output_dir, 'income_saving_diagram.svg'), format='svg', transparent=True)
plt.close()
